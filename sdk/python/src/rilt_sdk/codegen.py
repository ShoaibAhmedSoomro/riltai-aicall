...
The generated files live under `rilt_sdk.typed` and are committed to
the repository so `pip install rilt-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m rilt_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/rilt_sdk/typed

    python -m rilt_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from rilt_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from rilt_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from rilt_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m rilt_sdk.codegen",
        description="Generate typed SDK dataclasses from the AICall node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="AICall backend base URL")
