...
- `scripts/`
- `docs/`
- `pipecat/`, which is the upstream Pipecat git submodule; Rilt-owned
...
- top-level FastAPI wiring lives in `api/app.py`: it mounts the REST router
  under `/api/v1` and the AICall MCP server under `/api/v1/mcp`
...
- Rilt-specific realtime provider adapters live under `api/services/pipecat/realtime/`
...
- committed language-SDK output lives under `sdk/python/src/rilt_sdk/` and
  `sdk/typescript/src/`; the UI client under `ui/src/client/` is generated
  separately with the `ui` package's `generate-client` script

### AICall MCP server

Do not confuse the Rilt-hosted MCP API with customer-configured MCP tools used
inside a workflow.
...
- in AICall, common examples include telephony, workflow execution, the AICall
