...
- `scripts/`
- `docs/`
- `pipecat/`, which is the upstream Pipecat git submodule; Social-Cangaroo-owned
...
- top-level FastAPI wiring lives in `api/app.py`: it mounts the REST router
  under `/api/v1` and the Social Cangaroo MCP server under `/api/v1/mcp`
...
- Social-Cangaroo-specific realtime provider adapters live under `api/services/pipecat/realtime/`
...
- committed language-SDK output lives under `sdk/python/src/social_cangaroo_sdk/` and
  `sdk/typescript/src/`; the UI client under `ui/src/client/` is generated
  separately with the `ui` package's `generate-client` script

### Social Cangaroo MCP server

Do not confuse the Social-Cangaroo-hosted MCP API with customer-configured MCP tools used
inside a workflow.
...
- in Social Cangaroo, common examples include telephony, workflow execution, the Social Cangaroo
