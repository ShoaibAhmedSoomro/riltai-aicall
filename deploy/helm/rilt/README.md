...
Deploys AICall on Kubernetes with decomposed backend workloads (web,
...
|------------------------------|-------------|-----------------|-------|
| `rilt-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `rilt-arq-worker`          | 1 (HPA opt) | RollingUpdate   | Stateless |
| `rilt-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `rilt-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `rilt-ui`                  | 2 (HPA opt) | RollingUpdate   | Next.js SSR |
| `rilt-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
