...
{{- define "rilt.labels" -}}
helm.sh/chart: {{ include "rilt.chart" . }}
{{ include "rilt.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{- define "rilt.selectorLabels" -}}
app.kubernetes.io/name: {{ include "rilt.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}

{{- define "rilt.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "rilt.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
*/}}
{{- define "rilt.web.fullname" -}}{{ include "rilt.fullname" . }}-web{{- end }}
{{- define "rilt.arqWorker.fullname" -}}{{ include "rilt.fullname" . }}-arq-worker{{- end }}
{{- define "rilt.ariManager.fullname" -}}{{ include "rilt.fullname" . }}-ari-manager{{- end }}
{{- define "rilt.campaignOrchestrator.fullname" -}}{{ include "rilt.fullname" . }}-campaign-orchestrator{{- end }}
{{- define "rilt.ui.fullname" -}}{{ include "rilt.fullname" . }}-ui{{- end }}
{{- define "rilt.coturn.fullname" -}}{{ include "rilt.fullname" . }}-coturn{{- end }}
{{- define "rilt.migrate.fullname" -}}{{ include "rilt.fullname" . }}-migrate{{- end }}

{{- define "rilt.configMapName" -}}{{ include "rilt.fullname" . }}-config{{- end }}
{{- define "rilt.secretName" -}}
{{- if .Values.secrets.existingSecret -}}
{{- .Values.secrets.existingSecret -}}
{{- else -}}
{{- include "rilt.fullname" . }}-secret
{{- end -}}
{{- end }}
...
Called from each template via `include "rilt.deps.resolved" .` (no-op output).
*/}}
{{- define "rilt.deps.resolved" -}}
...
*/}}
{{- define "rilt.postgresHost" -}}{{ .Release.Name }}-postgresql{{- end }}
{{- define "rilt.redisHost" -}}{{ .Release.Name }}-redisinternal-master{{- end }}
{{- define "rilt.minioHost" -}}{{ .Release.Name }}-minio{{- end }}
...
password in the <release>-postgresql Secret under key `password`; rilt.dbEnv
projects it into $(POSTGRES_PASSWORD), which this URL interpolates at runtime.
Auth username/database default to `rilt` (see values.postgresql.auth).
*/}}
{{- define "rilt.databaseUrl" -}}
{{- if eq .Values.database.mode "internal" -}}
postgresql+asyncpg://{{ .Values.postgresql.auth.username }}:$(POSTGRES_PASSWORD)@{{ include "rilt.postgresHost" . }}:5432/{{ .Values.postgresql.auth.database }}
{{- else -}}
$(DATABASE_URL)
{{- end -}}
{{- end }}

{{- define "rilt.redisUrl" -}}
{{- if eq .Values.redis.mode "internal" -}}
redis://:$(REDIS_PASSWORD)@{{ include "rilt.redisHost" . }}:6379
{{- else -}}
$(REDIS_URL)
{{- end -}}
{{- end }}
...
embed $(POSTGRES_PASSWORD) / $(REDIS_PASSWORD) (see rilt.databaseUrl /
rilt.redisUrl), so the password vars must come first or the composed URLs
...
{{- end }}
- name: DATABASE_URL
  value: {{ include "rilt.databaseUrl" . | quote }}
- name: REDIS_URL
  value: {{ include "rilt.redisUrl" . | quote }}
...
*/}}
{{- define "rilt.backendEnvFrom" -}}
- configMapRef:
    name: {{ include "rilt.configMapName" . }}
- secretRef:
    name: {{ include "rilt.secretName" . }}
{{- end }}
