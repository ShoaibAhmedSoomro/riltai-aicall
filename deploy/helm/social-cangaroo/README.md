# Social Cangaroo Helm chart

Deploys Social Cangaroo on Kubernetes with decomposed backend workloads (web,
...
```bash
cd deploy/helm/social-cangaroo
...
|------------------------------|-------------|-----------------|-------|
| `social-cangaroo-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `social-cangaroo-arq-worker`          | 1 (HPA opt) | RollingUpdate   | Stateless |
| `social-cangaroo-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `social-cangaroo-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `social-cangaroo-ui`                  | 2 (HPA opt) | RollingUpdate   | Next.js SSR |
| `social-cangaroo-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
