...
ENV_FILE=".env"
REGISTRY="${REGISTRY:-ghcr.io/ShoaibAhmedSoomro}"
...
sync_postgres_password() {
    ...
    printf '%s\n' "ALTER USER postgres WITH PASSWORD :'social_cangaroo_password';" \
        | docker compose exec -T postgres psql \
        -U postgres \
        -d postgres \
        -v ON_ERROR_STOP=1 \
        -v "social_cangaroo_password=$postgres_password" >/dev/null
...
if [[ ! -t 0 ]]; then
    echo "Run the command above from an interactive shell to start Social Cangaroo."
    exit 0
fi

read -r -p "Start Social Cangaroo now? [Y/n]: " answer
case "$answer" in
    [Nn]*)
        echo "Social Cangaroo was not started."
        exit 0
        ;;
esac
