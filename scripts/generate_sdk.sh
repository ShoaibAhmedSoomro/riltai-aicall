...
# Requires:
#   - `python` in the `social-cangaroo` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t social-cangaroo-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t social-cangaroo-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m social_cangaroo_sdk.codegen \
    --input "$SPECS_JSON" \
...
datamodel-codegen \
    ...
    --output "sdk/python/src/social_cangaroo_sdk/_generated_models.py" \
