...
# Requires:
#   - `python` in the `rilt` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t rilt-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t rilt-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m rilt_sdk.codegen \
    --input "$SPECS_JSON" \
...
datamodel-codegen \
    ...
    --output "sdk/python/src/rilt_sdk/_generated_models.py" \
