...
# Registered BEFORE /{campaign_id}. FastAPI matches routes in declaration
# order, so a literal path that sits after a parameterised one on the same
# prefix is unreachable: /queue-summary binds campaign_id="queue-summary",
# fails int coercion and answers 422. It looks like a broken endpoint, not a
# routing mistake, which is why it survived review.
