...
def fake_docs_root(tmp_path: Path) -> Path:
    ...
    (docs_root / "getting-started" / "index.mdx").write_text(
        ...
        'description: "Start using AICall."\n'
        "---\n\n"
        "# Getting started\n\n"
        "Welcome to AICall.\n",
        encoding="utf-8",
    ...
    (docs_root / "deployment" / "docker.mdx").write_text(
        "---\n"
        'title: "Docker"\n'
        'description: "Deploy AICall with Docker."\n'
...
""",
    ...
    _clear_docs_caches()
    with patch.dict(os.environ, {"RILT_DOCS_PATH": str(docs_root)}):
        yield docs_root
    _clear_docs_caches()
...
def test_resolve_docs_root_honors_env_override(tmp_path: Path):
    ...
    with patch.dict(os.environ, {"RILT_DOCS_PATH": str(docs)}):
...
async def test_search_docs_returns_empty_when_no_corpus(
    ...
    monkeypatch.setenv("RILT_DOCS_PATH", str(nonexistent))
    _clear_docs_caches()
    with patch(
...
async def test_search_docs_rejects_zero_limit(fake_docs_root, authed_user):
    with pytest.raises(ValueError, match="at least 1"):
        await search_docs("AICall", limit=0)
...
async def test_read_doc_can_target_section(fake_docs_root, authed_user):
    ...
    assert "Run AICall with Docker." not in result["content"]
