...
def test_rilt_llm_uses_explicit_mps_correlation_id():
    service = RiltLLMService(
        api_key="mps-secret",
...
@pytest.mark.asyncio
async def test_rilt_stt_config_uses_explicit_mps_correlation_id(monkeypatch):
...
@pytest.mark.asyncio
async def test_rilt_tts_messages_use_explicit_mps_correlation_id(monkeypatch):
    fake_ws = _FakeWebSocket()
...
@pytest.mark.asyncio
async def test_rilt_tts_final_for_missing_context_is_ignored():
    service = RiltTTSService(api_key="mps-secret")
