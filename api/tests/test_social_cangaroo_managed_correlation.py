...
from pipecat.services.dograh.stt import DograhSTTService as SocialCangarooSTTService
from pipecat.services.dograh.tts import DograhTTSService as SocialCangarooTTSService
...
def test_social_cangaroo_llm_uses_explicit_mps_correlation_id():
    service = SocialCangarooLLMService(
        api_key="mps-secret",
...
@pytest.mark.asyncio
async def test_social_cangaroo_stt_config_uses_explicit_mps_correlation_id(monkeypatch):
    ...
    service = SocialCangarooSTTService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=16000,
    )
...
@pytest.mark.asyncio
async def test_social_cangaroo_tts_messages_use_explicit_mps_correlation_id(monkeypatch):
    fake_ws = _FakeWebSocket()
    ...
    service = SocialCangarooTTSService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=24000,
    )
...
@pytest.mark.asyncio
async def test_social_cangaroo_tts_final_for_missing_context_is_ignored():
    service = SocialCangarooTTSService(api_key="mps-secret")
