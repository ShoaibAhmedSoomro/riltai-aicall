"""Tests for the Social-Cangaroo-managed embedding service and its correlation resolver."""
...
from api.services.gen_ai.embedding.social_cangaroo_service import SocialCangarooEmbeddingService
...
def _service_with_fake_client(correlation_id):
    service = SocialCangarooEmbeddingService(
        db_client=None,
        api_key="sk-test",
...
@pytest.mark.asyncio
async def test_social_cangaroo_embedding_forwards_v2_protocol_when_correlation_present():
...
@pytest.mark.asyncio
async def test_social_cangaroo_embedding_sends_plain_without_correlation():
