---
title: "Update"
description: "Update your self-hosted AICall stack to a newer image version"
---

This guide covers updating a AICall stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `rilt/` directory if you used `setup_remote.sh`).
...
AICall publishes two images — `rilt-api` and `rilt-ui` — to both container registries:

- **GitHub Container Registry** — [github.com/orgs/ShoaibAhmedSoomro/packages](https://github.com/orgs/ShoaibAhmedSoomro/packages)
- **Docker Hub** — [hub.docker.com/u/riltai](https://hub.docker.com/u/riltai)
...
|-------|-----------|---------|-------------|
| GitHub release tag | `rilt-vX.Y.Z` | `rilt-v1.28.0` | Use when copying the version from a GitHub release |
...
<Warning>
Always update **`rilt-api`** and **`rilt-ui`** to the **same tag**. The two images are built from the same commit and the UI expects API responses in a matching shape — mixing versions will break the app. `update_remote.sh` handles this for you automatically.
</Warning>
...
- Synchronizes the canonical remote keys in `.env` and validates the runtime config that `rilt-init` will render from it.
...
```bash
cd rilt
curl -o update_remote.sh https://raw.githubusercontent.com/ShoaibAhmedSoomro/riltai-aicall/main/scripts/update_remote.sh
bash update_remote.sh
```

You'll be prompted for the target version, defaulting to the most recent release. Accepted forms: bare semver (`1.28.0`), v-prefixed (`v1.28.0`), the full GitHub tag (`rilt-v1.28.0`), or `main` to refresh deployment files from the default branch — the script normalizes them. Non-interactive callers can set it via environment variable and skip the confirmation prompt:

```bash
TARGET_VERSION=1.28.0 RILT_UPDATE_YES=1 bash update_remote.sh
```
...
<CodeGroup>
```bash macOS/Linux
curl -o docker-compose.yaml https://raw.githubusercontent.com/ShoaibAhmedSoomro/riltai-aicall/main/docker-compose.yaml
curl -o start_docker.sh https://raw.githubusercontent.com/ShoaibAhmedSoomro/riltai-aicall/main/scripts/start_docker.sh && chmod +x start_docker.sh
docker compose down
./start_docker.sh
```
```powershell Windows
Invoke-WebRequest -OutFile docker-compose.yaml https://raw.githubusercontent.com/ShoaibAhmedSoomro/riltai-aicall/main/docker-compose.yaml; Invoke-WebRequest -OutFile start_docker.ps1 https://raw.githubusercontent.com/ShoaibAhmedSoomro/riltai-aicall/main/scripts/start_docker.ps1; docker compose down; .\start_docker.ps1
```
</CodeGroup>
...
Rolling back restores **configuration files only** — it does nothing for your data. Postgres, Redis and MinIO live in Docker volumes on the host, so losing the host loses every agent, call, credential and recording. Set this up before you need it:

```bash
./scripts/backup.sh backup
```

That dumps Postgres and archives the MinIO volume into `./backups/<timestamp>/`, checks the dump is valid, and prunes anything older than `BACKUP_KEEP_DAYS` (default 14). Redis is intentionally skipped — it holds the job queue and rate-limit counters, all rebuildable, and restoring stale in-flight jobs on top of a restored database is worse than losing them.

Run it nightly:

```bash
0 3 * * * cd /path/to/rilt && ./scripts/backup.sh backup >> backups/backup.log 2>&1
```

<Warning>
A backup on the same host is not a backup. Set `BACKUP_POST_HOOK` to copy it somewhere else — it receives the backup directory as `$1`, so any tool works and no credentials are baked into the repo:

```bash
BACKUP_POST_HOOK='rclone copy "$1" r2:aicall-backups/' ./scripts/backup.sh backup
```
</Warning>

**Prove it works before you rely on it.** This restores into a throwaway container and counts what came back; it never touches your live database:

```bash
./scripts/backup.sh verify backups/<timestamp>
```

To restore for real (this overwrites the live database and object storage, and asks for confirmation first):
...
```bash
cd rilt
for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_rilt_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/ShoaibAhmedSoomro/riltai-aicall/issues) before rolling back.
</Warning>
...
# Track latest main:
git pull
# Or pin to a specific release (git tag format is rilt-vX.Y.Z):
git checkout rilt-v1.28.0
