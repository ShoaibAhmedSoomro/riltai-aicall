...
sequenceDiagram
    ...
    participant API as Social Cangaroo API
...
The phone infrastructure. Social Cangaroo connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and Social Cangaroo in real time.

**[Transcriber (STT)](/configurations/transcriber)**
Converts the caller's speech to text in real time. Social Cangaroo sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. Social Cangaroo instructs your telephony provider to dial the number
...
6. When an edge condition is met, Social Cangaroo transitions to the next node
