...
description: "Configure the speech-to-speech, Social-Cangaroo-managed, or bring-your-own-key models your Social Cangaroo agents use."
---
...
To configure models, open **Models** in your Social Cangaroo dashboard:

- **Hosted:** `https://app.socialcangaroo.com/model-configurations`
...
| **Social Cangaroo** | Use Social-Cangaroo-managed LLM, voice, and transcriber models behind one Social Cangaroo Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but Social Cangaroo uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## Social Cangaroo

Use **Social Cangaroo** when you want Social Cangaroo to manage the model providers for you. This path uses one Social Cangaroo Service Key for Social-Cangaroo-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![Social Cangaroo model configuration](../images/model-configuration-social-cangaroo.png)

Configure:

| Field | What it controls |
|-------|------------------|
| **Voice** | The Social-Cangaroo-managed voice to use. |
| **Speed** | The voice playback speed. |
| **Language** | The language behavior, including multilingual auto-detect when available. |
| **API Key** | Your Social Cangaroo Service Key. Create Service Keys from **Developers**. |
