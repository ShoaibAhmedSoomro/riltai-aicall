...
description: "Configure the speech-to-speech, RiltAI-managed, or bring-your-own-key models your AICall agents use."
---
...
To configure models, open **Models** in your AICall dashboard:

- **Hosted:** `https://aicall.rilt.ai/model-configurations`
...
| **AICall** | Use RiltAI-managed LLM, voice, and transcriber models behind one AICall Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but AICall uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## AICall

Use **AICall** when you want AICall to manage the model providers for you. This path uses one AICall Service Key for RiltAI-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![AICall model configuration](../images/model-configuration-rilt.png)

Configure:

| Field | What it controls |
|-------|------------------|
| **Voice** | The RiltAI-managed voice to use. |
| **Speed** | The voice playback speed. |
| **Language** | The language behavior, including multilingual auto-detect when available. |
| **API Key** | Your AICall Service Key. Create Service Keys from **Developers**. |
