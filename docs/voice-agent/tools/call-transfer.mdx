...
When the LLM decides a transfer is needed, it calls the Call Transfer tool. Social Cangaroo resolves the destination, validates the transfer setup, and then starts the provider transfer.

For a successful transfer:

1. The agent calls the Call Transfer tool.
2. Social Cangaroo resolves the destination from static configuration, ordered context mappings, or a dynamic HTTP resolver.
3. Social Cangaroo optionally plays a pre-transfer message.
4. Social Cangaroo starts dialing the destination through the telephony provider.
5. The caller hears hold music while the destination leg is connecting.
6. When the destination answers, Social Cangaroo connects the caller and ends the AI agent's involvement.

The transfer is a blind transfer. Social Cangaroo does not currently pass conversation context to the receiving human as part of the transfer.
...
Use this when a finite set of context values should select the transfer destination. You can add multiple rules and order them. Social Cangaroo evaluates the rules from top to bottom and uses the first route whose value matches, ignoring case and surrounding whitespace.
...
Social Cangaroo performs final variable extraction before evaluating context mappings. If no rule matches, it uses the optional fallback destination. If there is no fallback, or a selected destination template resolves to an empty value, the transfer fails before dialing and the agent can continue the call.

### Dynamic HTTP Resolver

Use this when the destination must be decided during the conversation.

Social Cangaroo sends a `POST` request to your resolver endpoint before starting the transfer. Your endpoint returns the destination and may return a custom message to play before dialing.
...
- Use business logic from your backend instead of hardcoding numbers in Social Cangaroo

## Dynamic resolver request

The resolver request body is a flat JSON object.

Social Cangaroo builds it from:

- **LLM parameters**: values the agent extracts from the conversation when it calls the transfer tool
- **Preset parameters**: values Social Cangaroo injects from fixed values or templates such as `{{initial_context.account_id}}` or `{{gathered_context.billing_issue_type}}`
...
<Note>
Social Cangaroo does not send the full conversation transcript to the resolver. Add the specific values you need as LLM parameters or preset parameters.
</Note>
...
<Warning>
Follow this response shape exactly. Social Cangaroo reads specific keys from the resolver response. For dynamic transfer resolution, an invalid response does not fall back silently: the transfer fails before dialing.
</Warning>
...
- **Resolver URL**: HTTPS or HTTP endpoint Social Cangaroo calls with `POST`
- **Resolver Timeout**: How long Social Cangaroo waits for the resolver response
- **Resolver Wait Message**: Optional message spoken while Social Cangaroo waits, such as "One moment while I find the right team."
- **LLM Parameters**: Values the agent should extract from the conversation, such as `billing_issue_type` or `requested_department`
- **Preset Parameters**: Values injected by Social Cangaroo from context or fixed values
...
1. Social Cangaroo resolves the static or template destination.
2. Social Cangaroo plays the configured pre-transfer message, if any.
3. Social Cangaroo starts the provider transfer.

For dynamic transfers:

1. Social Cangaroo may play the resolver wait message while calling your resolver.
2. If the resolver returns `custom_message`, Social Cangaroo plays that message.
3. If the resolver does not return `custom_message`, Social Cangaroo uses the configured pre-transfer message.
4. Social Cangaroo starts the provider transfer.

Resolver `custom_message` overrides the configured pre-transfer message for that transfer attempt.

## Transfer timeout

Transfer timeout is the maximum time Social Cangaroo waits for the destination to answer after the provider transfer starts.

This timeout applies to static, context-mapped, and dynamic transfers.

The resolver timeout is separate. It controls only how long Social Cangaroo waits for your HTTP resolver before dialing starts.
...
- a preset parameter, if Social Cangaroo should inject it from context
