...
description: Add your AICall agent to any website so visitors can talk to it by voice or chat with it by text.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="rilt-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio/chat pipeline plus a JavaScript API on `window.RiltWidget`.                | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `rilt-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.RiltWidget`. Code that registers callbacks must wait for the widget to be available.
...
```html
<!-- Paste the rilt embed snippet from the dashboard somewhere on the page -->
<div id="rilt-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.RiltWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('rilt-inline-container');
      if (window.RiltWidget && container) {
        const { isInitialized } = window.RiltWidget.getState();
        if (isInitialized) window.RiltWidget.refresh();
        else window.RiltWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="rilt-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.RiltWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.RiltWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.RiltWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.RiltWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.RiltWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.RiltWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.RiltWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.RiltWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |
| `window.RiltWidget.setContext(vars)`         | Merge visitor context for the next call — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless.         |
...
| Method / Callback                              | Description                                                                                                                                                  |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.RiltWidget.startChat()`              | Start a conversation. The agent's greeting arrives via `onMessage`.                                                                                          |
| `window.RiltWidget.sendMessage(text)`        | Send a visitor message. Returns a Promise that resolves with the updated transcript (array of turns), or `null` if the message couldn't be delivered.        |
| `window.RiltWidget.getMessages()`            | Current transcript as an array of turns: `{ id, status, user_message, assistant_message }`, each message being `{ text, created_at }`.                       |
| `window.RiltWidget.onMessage(cb)`            | Fires once per new agent reply. Callback receives `(text, turn)`.                                                                                            |
| `window.RiltWidget.onChatStateChange(cb)`    | Fires on every chat state change. States: `idle`, `starting`, `ready`, `waiting` (agent is replying), `ended`, `expired`, `error`.                           |
| `window.RiltWidget.onError(cb)`              | Fires on errors. Callback receives an `Error` object.                                                                                                        |
| `window.RiltWidget.setContext(vars)`         | Merge visitor context for the next conversation — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless. |
...
<script>
  window.addEventListener('load', () => {
    window.RiltWidget.onMessage((text) => {
    ...
    document.getElementById('send-btn').addEventListener('click', async () => {
      ...
      document.getElementById('transcript').appendChild(p);
      await window.RiltWidget.sendMessage(input.value);
      input.value = '';
    });
  });
</script>
...
<Note>
**About timing.** The widget script loads asynchronously, so `window.RiltWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.RiltWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.RiltWidget.onError((err) => {
    console.error('AICall error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.RiltWidget.onStatusChange((s) => setStatus(s));
    window.RiltWidget.onError((err) => console.error('AICall error:', err.message));
  }, []);
...
```js
window.RiltWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.RiltWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
