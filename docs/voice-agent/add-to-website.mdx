...
description: Add your Social Cangaroo agent to any website so visitors can talk to it by voice or chat with it by text.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="social-cangaroo-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio/chat pipeline plus a JavaScript API on `window.SocialCangarooWidget`.                | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `social-cangaroo-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.SocialCangarooWidget`. Code that registers callbacks must wait for the widget to be available.
...
```js
window.SocialCangarooWidget.setContext({
...
```html
<!-- Paste the social-cangaroo embed snippet from the dashboard somewhere on the page -->
<div id="social-cangaroo-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.SocialCangarooWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('social-cangaroo-inline-container');
      if (window.SocialCangarooWidget && container) {
        const { isInitialized } = window.SocialCangarooWidget.getState();
        if (isInitialized) window.SocialCangarooWidget.refresh();
        else window.SocialCangarooWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="social-cangaroo-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.SocialCangarooWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.SocialCangarooWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.SocialCangarooWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.SocialCangarooWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.SocialCangarooWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.SocialCangarooWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.SocialCangarooWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.SocialCangarooWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |
| `window.SocialCangarooWidget.setContext(vars)`         | Merge visitor context for the next call — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless.         |
...
| Method / Callback                              | Description                                                                                                                                                  |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.SocialCangarooWidget.startChat()`              | Start a conversation. The agent's greeting arrives via `onMessage`.                                                                                          |
| `window.SocialCangarooWidget.sendMessage(text)`        | Send a visitor message. Returns a Promise that resolves with the updated transcript (array of turns), or `null` if the message couldn't be delivered.        |
| `window.SocialCangarooWidget.getMessages()`            | Current transcript as an array of turns: `{ id, status, user_message, assistant_message }`, each message being `{ text, created_at }`.                       |
| `window.SocialCangarooWidget.onMessage(cb)`            | Fires once per new agent reply. Callback receives `(text, turn)`.                                                                                            |
| `window.SocialCangarooWidget.onChatStateChange(cb)`    | Fires on every chat state change. States: `idle`, `starting`, `ready`, `waiting` (agent is replying), `ended`, `expired`, `error`.                           |
| `window.SocialCangarooWidget.onError(cb)`              | Fires on errors. Callback receives an `Error` object.                                                                                                        |
| `window.SocialCangarooWidget.setContext(vars)`         | Merge visitor context for the next conversation — see [Pass context to the agent](#pass-context-to-the-agent). Works in every embed mode, not just headless. |
...
<script>
  window.addEventListener('load', () => {
    window.SocialCangarooWidget.onMessage((text) => {
    ...
    document.getElementById('open-chat').addEventListener('click', () => {
      window.SocialCangarooWidget.startChat();
    });
    ...
    document.getElementById('send-btn').addEventListener('click', async () => {
      ...
      document.getElementById('transcript').appendChild(p);
      await window.SocialCangarooWidget.sendMessage(input.value);
      input.value = '';
    });
  });
</script>
...
<Note>
**About timing.** The widget script loads asynchronously, so `window.SocialCangarooWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.SocialCangarooWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.SocialCangarooWidget.onError((err) => {
    console.error('Social Cangaroo error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.SocialCangarooWidget.onStatusChange((s) => setStatus(s));
    window.SocialCangarooWidget.onError((err) => console.error('Social Cangaroo error:', err.message));
  }, []);
...
```js
window.SocialCangarooWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.SocialCangarooWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
