...
2. Social Cangaroo sends a **POST** request to your configured endpoint with a standardized payload.
...
| **Endpoint URL** | The URL Social Cangaroo will send the POST request to. |
...
Social Cangaroo sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/social-cangaroo/pre-call", async (req, res) => {
