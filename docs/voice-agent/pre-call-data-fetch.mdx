...
2. AICall sends a **POST** request to your configured endpoint with a standardized payload.
...
| **Endpoint URL** | The URL AICall will send the POST request to. |
...
AICall sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/rilt/pre-call", async (req, res) => {
