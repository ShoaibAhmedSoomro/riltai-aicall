...
Choose whether Social Cangaroo makes a POST request before **all**, **inbound**, or **outbound** calls, or leave the fetch **disabled**. Social Cangaroo merges the JSON response into the call context as template variables before rendering the greeting and Start-node prompt.
