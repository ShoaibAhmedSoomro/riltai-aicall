...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, Social Cangaroo generates a unique endpoint URL that external systems can call to start a conversation.
...
When you add an API Trigger node to your workflow, Social Cangaroo assigns it a unique UUID. The trigger node exposes two URLs that share this UUID:
...
```
POST https://your-social-cangaroo-instance/api/v1/public/agent/{uuid}        # Production
POST https://your-social-cangaroo-instance/api/v1/public/agent/test/{uuid}   # Test
```

If you are using the hosted version, replace `your-social-cangaroo-instance` with `api.socialcangaroo.com`.
...
<CodeGroup>
```bash Production URL
curl -X POST https://your-social-cangaroo-instance/api/v1/public/agent/{uuid} \
...
```bash Test URL
curl -X POST https://your-social-cangaroo-instance/api/v1/public/agent/test/{uuid} \
  -H "Content-Type: application/json" \
  -H "X-API-Key: dg_your_api_key" \
  -d '{
    "phone_number": "+14155550100",
...
Pass `greeting_override` inside `initial_context` to replace the Start Call node's saved greeting for one API-triggered call. A valid override takes precedence over the saved greeting. If you omit it, Social Cangaroo uses the saved greeting; if neither exists, the Start node follows its normal no-greeting behavior.
...
The id is shown on each row in **Telephony configurations** (`https://app.socialcangaroo.com/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
