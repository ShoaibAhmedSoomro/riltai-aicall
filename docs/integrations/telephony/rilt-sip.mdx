---
title: "RiltAI Managed SIP"
description: "Connect your own SIP carrier or PBX to the SIP domain AICall provisions for your organization"
---

## What you get, and what you bring

Every AICall organization is provisioned with a telephony configuration named
**AICall Cloudonix SIP**. It gives you a SIP domain that is already connected
to the AICall voice pipeline — you do not need a Cloudonix account, and there
is nothing to pay for or set up on the Cloudonix side.

What it does **not** include is phone service. AICall does not sell or resell
phone numbers, and no calls flow until you connect a carrier:

| AICall provides                                     | You provide                                    |
| --------------------------------------------------- | ---------------------------------------------- |
...
<Note>
  Until you finish the steps below, **AICall Cloudonix SIP** cannot place a
...
## Step 1: Read your SIP endpoints from AICall

1. Go to **Telephony configurations** and open **AICall Cloudonix SIP**
...
4. Note the two values you will need on the carrier side:
   - **Inbound hostname** and **ports** — where your carrier sends calls *to* AICall
   - **Outbound origin IP address** — the address AICall's calls arrive *from*

## Step 2: Point your carrier at AICall (inbound)
...
AICall needs to know where to send calls when your agent dials out.

1. On your carrier, allow the **origin IP address** from Step 1 to send SIP
   traffic to your trunk. Calls from AICall are unauthenticated and identified
   by source IP, so this step is required
2. Back in AICall, under **Outbound trunks**, choose **Add trunk** and fill in:
   - **Name** — any name you choose. Letters, digits and hyphens only
   - **SIP domain** — the hostname of your carrier or PBX, for example
     `sip.example.com`. AICall puts this in both the SIP `To` header and the
     Request-URI
   - **Region** — the edge AICall dials your carrier from. The dialog shows
...
rejected by the carrier even though AICall accepts it — which is why the trunk
assignment matters as soon as you have more than one carrier. With a single
trunk, AICall uses it for every number and the field can be left alone.
...
AICall pushes the inbound webhook URL to the Voice Application on the
...
2. Choose **Phone Call**
3. Select **AICall Cloudonix SIP**, pick your caller ID, enter a destination
