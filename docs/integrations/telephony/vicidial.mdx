...
description: "Connect AICall to VICIdial through Asterisk ARI for call control, in-group transfers, and lead updates"
---

## Overview

Use this integration when VICIdial owns the customer call and connects AICall
to it through Asterisk. AICall uses Asterisk ARI for audio and VICIdial APIs for
...
1. Open **Platform Settings** by going to https://aicall.rilt.ai/settings
...
telephony configuration and are returned masked by the AICall API.

## Pass VICIdial call identity to Asterisk

AICall needs VICIdial's call identity to control the original customer leg.
Configure VICIdial or the connecting dialplan to preserve these SIP headers on
the call that enters the AICall Stasis application:
...
Each lead field is then read individually during call setup, so AICall captures
...
applies only after every rule misses. The match is case-insensitive. AICall
...
does not prevent AICall from attempting the requested transfer or hangup.

## Runtime behavior

When the workflow transfers the call, AICall resolves the configured in-group,
...
When the conversation ends without a transfer, AICall persists final gathered
