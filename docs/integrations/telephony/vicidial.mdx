...
description: "Connect Social Cangaroo to VICIdial through Asterisk ARI for call control, in-group transfers, and lead updates"
---

## Overview

Use this integration when VICIdial owns the customer call and connects Social Cangaroo
to it through Asterisk. Social Cangaroo uses Asterisk ARI for audio and VICIdial APIs for
...
1. Open **Platform Settings** by going to https://app.socialcangaroo.com/settings
...
telephony configuration and are returned masked by the Social Cangaroo API.

## Pass VICIdial call identity to Asterisk

Social Cangaroo needs VICIdial's call identity to control the original customer leg.
Configure VICIdial or the connecting dialplan to preserve these SIP headers on
the call that enters the Social Cangaroo Stasis application:
...
Each lead field is then read individually during call setup, so Social Cangaroo captures
...
applies only after every rule misses. The match is case-insensitive. Social Cangaroo
...
does not prevent Social Cangaroo from attempting the requested transfer or hangup.

## Runtime behavior

When the workflow transfers the call, Social Cangaroo resolves the configured in-group,
...
When the conversation ends without a transfer, Social Cangaroo persists final gathered
