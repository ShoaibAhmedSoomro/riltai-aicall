...
description: "Trigger a Social Cangaroo voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the Social Cangaroo UI.

## Prerequisites

- A Social Cangaroo [API key](/configurations/api-keys) exported as `SOCIAL_CANGAROO_API_KEY`
...
with SocialCangarooClient(api_key="YOUR_API_KEY") as client:
...
const client = new SocialCangarooClient({ apiKey: "YOUR_API_KEY" });
