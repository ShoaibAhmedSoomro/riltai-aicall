...
description: "Trigger a AICall voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the AICall UI.

## Prerequisites

- A AICall [API key](/configurations/api-keys) exported as `RILT_API_KEY`
...
with RiltClient(api_key="YOUR_API_KEY") as client:
...
const client = new RiltClient({ apiKey: "YOUR_API_KEY" });
