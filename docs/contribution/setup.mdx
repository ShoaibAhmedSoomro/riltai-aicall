---
title: Setup
description: Get a Social Cangaroo dev environment running and learn the daily commands.
---

Social Cangaroo is a Next.js UI (`ui/`), a FastAPI backend (`api/`), and a [Pipecat](https://github.com/pipecat-ai/pipecat)-based voice pipeline (`pipecat/`, a git submodule), backed by Postgres, Redis, and MinIO.

<Tip>
**Using Claude Code or Codex?** Install the [Social Cangaroo setup skill](https://github.com/ShoaibAhmedSoomro/social-cangaroo-plugins) (`/plugin marketplace add ShoaibAhmedSoomro/social-cangaroo-plugins`, then `/plugin install social-cangaroo@social-cangaroo`) and ask your agent to *"set up Social Cangaroo for development"* — it runs the steps below for you and verifies the stack is healthy.
</Tip>
...
1. Fork [ShoaibAhmedSoomro/social-cangaroo](https://github.com/ShoaibAhmedSoomro/social-cangaroo) and clone **your fork**:

```bash
git clone https://github.com/<YOUR_HANDLE>/social-cangaroo
cd social-cangaroo
```
...
If these steps do not work for you, please open an issue on [GitHub](https://github.com/ShoaibAhmedSoomro/social-cangaroo/issues).

<AccordionGroup>
<Accordion title="Cloned ShoaibAhmedSoomro/social-cangaroo directly instead of your fork?">
Run `bash scripts/setup_fork.sh` once. It prompts for your fork URL, points `origin` at it, and adds `upstream` as `ShoaibAhmedSoomro/social-cangaroo`. See [Fork and upstream remotes](/contribution/reference#fork-and-upstream-remotes).
</Accordion>
...
| `sdk/` | Python/TypeScript SDKs for driving Social Cangaroo programmatically |
...
2. Push to your fork (`origin`) and open a pull request against `ShoaibAhmedSoomro/social-cangaroo:main`.
3. A maintainer reviews and merges.

Bug reports and feature ideas go through [GitHub Issues](https://github.com/ShoaibAhmedSoomro/social-cangaroo/issues) and [Ideas](https://github.com/orgs/ShoaibAhmedSoomro/discussions/categories/ideas). Issues tagged `good first issue` are a good place to start. For questions while you're working, use the [Social Cangaroo Community Slack](https://join.slack.com/t/social-cangaroo-community/shared_invite/zt-4787daqcn-3TDiQUh~3xrr3pwAqR9wpQ).
