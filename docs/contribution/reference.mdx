...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `ShoaibAhmedSoomro/rilt`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
- `upstream`: `ShoaibAhmedSoomro/rilt`, where new commits land

If you cloned `ShoaibAhmedSoomro/rilt` directly instead of your fork, run it once (inside the devcontainer is fine):
...
<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `ShoaibAhmedSoomro/rilt:main`. Never push directly to `upstream`.
</Note>
