...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `ShoaibAhmedSoomro/riltai-aicall`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, please open a [GitHub issue](https://github.com/ShoaibAhmedSoomro/riltai-aicall/issues).
...
- `upstream`: `ShoaibAhmedSoomro/riltai-aicall`, where new commits land

If you cloned `ShoaibAhmedSoomro/riltai-aicall` directly instead of your fork, run it once (inside the devcontainer is fine):
...
```bash
origin    https://github.com/<YOUR_HANDLE>/rilt.git (fetch/push)
upstream  https://github.com/ShoaibAhmedSoomro/riltai-aicall.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `ShoaibAhmedSoomro/riltai-aicall:main`. Never push directly to `upstream`.
</Note>
