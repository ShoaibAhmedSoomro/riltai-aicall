...
description: "Complete reference for all environment variables used by the AICall backend"
---
...
AICall supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get AICall running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through RiltAI Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
Password resets and organization invitations are delivered through [Resend](https://resend.com). Both variables must be set before anything is sent.

| Variable | Default | Description |
|---|---|---|
| `RESEND_API_KEY` | unset | Resend API key. Without it, no mail is sent |
| `EMAIL_FROM` | unset | Sender address, e.g. `AICall <noreply@yourdomain.com>`. Must be on a domain **verified in Resend** |
| `PASSWORD_RESET_EXPIRY_MINUTES` | `30` | How long a reset link stays usable. It is also single use |

Leave them unset and the product says so rather than pretending: `POST /auth/forgot-password` answers `503`, and the invitations screen keeps its "recorded but not yet delivered" notice. Nothing fails silently.

<Note>
A verified sender is the usual sticking point. An otherwise valid key returns `403` when `EMAIL_FROM` is on a domain Resend has not verified for your account.
</Note>
...
| `MPS_API_URL` | `https://services.dograh.com` | RiltAI Managed Platform Services URL |
| `RILT_MPS_SECRET_KEY` | `null` | **Required for non-OSS deployments.** Secret key for authenticating with MPS |
...
AICall uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
...
<Note>
Presigned URLs point at `S3_ENDPOINT_URL`, so that host must be reachable from the browser. Because browsers fetch transcripts cross-origin, the bucket also needs a CORS rule allowing your app's origin for `GET`/`HEAD` — configure this on the storage server (e.g. via `PutBucketCors`), not in AICall.
</Note>
...
Carriers dial the media WebSocket back at `/api/v1/telephony/ws/{workflow_id}/{organization_id}/{workflow_run_id}`. Those ids travel in a caller-visible URL, so on their own they are a guessable capability. Setting a secret makes AICall sign that URL with an HMAC and verify the signature when the socket opens.
...
Controls concurrency for [Campaigns](/core-concepts/campaigns), AICall's bulk outbound calling feature.
