...
description: "Context variables available in webhook nodes and the data AICall sends after a call"
---

AICall executes **webhook nodes** asynchronously after a workflow run completes. You configure the target URL, HTTP method, headers, and payload template directly in the workflow definition.
...
2. AICall executes any `webhook` nodes in the workflow asynchronously
...
<Note>
AICall always includes a top-level `call_disposition` field in the delivered payload. If your template doesn't set one, it is added automatically from `gathered_context.call_disposition`; if your template sets it explicitly, your value is kept.
</Note>
...
@app.post("/webhook/rilt")
async def handle_rilt_webhook(request: Request):
...
```json
{
  ...
  "data": {
    ...
    "enabled": true,
    "http_method": "POST",
    "endpoint_url": "https://your-service.com/webhook/rilt",
    "payload_template": {
      "run_id": "{{workflow_run_id}}",
      "customer": "{{initial_context.customer_name}}",
      "outcome": "{{gathered_context.resolution}}"
