...
<p align="center">
  <a href="https://aicall.rilt.ai">
    <img src="https://img.shields.io/badge/▶_体验云端版本-aicall.rilt.ai-2563eb?style=for-the-badge" alt="体验云端版本">
  </a>
  &nbsp;
  <a href="#-快速开始">
...
<p align="center">
  <a href="https://docs.rilt.ai">📖 文档</a> &nbsp;·&nbsp;
...
<p align="center">
  <img src="docs/images/hero.gif" alt="AICall 实战演示 —— 搭建工作流、启动语音智能体、直接对话" width="80%">
</p>
...
| **自带 LLM / STT / TTS** | ✅ 任意厂商,也可使用 AICall 自带方案 | 在其集成范围内可配置 | 在其集成范围内可配置 |
...
> **⚡ 想让 AI 智能体帮你完成部署?**
> 如果你使用 **Claude Code** 或 **Codex**,可以安装官方的 [AICall 部署技能(skill)](https://github.com/ShoaibAhmedSoomro/riltai-aicall-plugins),让智能体替你完成安装、配置与排障——它会识别你的操作系统、选择合适的部署方式、运行 AICall 自带的部署脚本并验证结果。
>
> ```text
> # 在 Claude Code 中
...
> ```
>
> 然后开启一个新会话,让它 _"set up AICall"_(或运行 `/rilt-setup`)。Codex 同样支持——详见[插件仓库](https://github.com/ShoaibAhmedSoomro/riltai-aicall-plugins#install)。
...
3. 点击 **Test Agent**。
4. 使用 **Test Audio** 在浏览器中和智能体语音对话,或使用 **Test Chat** 通过文本快速迭代。在 Test Chat 中,你可以编辑或重放用户消息,AICall 会从该位置重新生成智能体回复和节点流转。

> 🔑 **无需 API Key。** AICall 自带一套自动生成的密钥,以及内置的 LLM / TTS / STT 栈。你可以随时接入自己的 LLM、TTS、STT 或电信服务商(如 Twilio、Vonage、Telnyx)。

## 使用 MCP 构建智能体

AICall 内置 MCP 服务器,因此编码智能体可以直接在你的 AICall 工作区中操作。

连接 Codex、Claude Code、Cursor 或任何 MCP 客户端后,可以查看现有智能体、搜索 AICall 文档、获取节点 schema、创建新工作流,并通过自然语言保存草稿修改。

让编码智能体构建语音智能体时,请分享一份面向该用例的简短脚本,而不是只给一行提示。脚本最好包含智能体 persona、通话流程、规则、异议处理、成功标准,以及可选的示例对话。

请参见 [MCP 指南](https://docs.rilt.ai/integrations/mcp) 来连接你的助手。
...
参见[本地部署](https://docs.rilt.ai/contribution/setup)。

### 自托管部署

如需了解远程服务器部署及 HTTPS 配置的详细步骤,请参见我们的 [Docker 部署指南](https://docs.rilt.ai/deployment/docker#option-2-remote-server-deployment)。

### 云端版本

托管云版本请访问 [https://www.rilt.ai](https://www.rilt.ai/)。

## 📚 文档

完整文档请访问 [https://docs.rilt.ai](https://docs.rilt.ai/)。
...
我们欢迎一切贡献!AICall 100% 开源,也会一直保持下去。
